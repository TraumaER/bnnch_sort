 ...
 *
 * <p>Items are grouped by the creative mode tab they belong to, with items in earlier tabs sorting
 * before items in later tabs. Items within the same tab are sorted alphabetically. Uncategorized
 * items (not in any tab) sort last, ordered by their registry ID.
 *
 * <h2>Sort Order</h2>
 *
 * <ol>
 *   <li>Items are grouped by creative tab index (registration order)
 *   <li>Within each tab, items are sorted alphabetically by name
 *   <li>Items not in any tab sort last, by registry ID
 * </ol>
 *
 * <h2>Usage</h2>
 *
...
public class CategoryComparator implements Comparator<ItemStack> {
   ...
   *
   * <p>Items are first compared by their creative tab index. If both items are in the same tab (or
   * both uncategorized), they are compared alphabetically.
   *
   ...
   *
   * <p>Items in creative tabs return the tab's index (0-based). Items not in any tab return {@link
   * #UNCATEGORIZED_OFFSET} plus their registry ID to ensure consistent ordering.
   *
