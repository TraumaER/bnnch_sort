 ...
 *
 * <p>Items are grouped by the mod that registered them, with vanilla Minecraft items (namespace
 * "minecraft") typically sorting first. Items from the same mod are sorted alphabetically by their
 * display name.
 *
 * <h2>Sort Order</h2>
 *
 * <ol>
 *   <li>Items are grouped by mod namespace (case-insensitive)
 *   <li>Within each mod, items are sorted alphabetically by name
 * </ol>
 *
 * <h2>Usage</h2>
 *
...
public class ModIdComparator implements Comparator<ItemStack> {
   ...
   *
   * <p>Items are first compared by their registry namespace (mod ID) using case-insensitive
   * comparison. If both items are from the same mod, they are compared alphabetically by display
   * name.
   *
