 ...
 *
 * <p>This class provides a simple overlay message system that displays temporary feedback when
 * sorting is performed or preferences are changed. Messages are displayed for a fixed duration
 * before automatically expiring.
 *
 * <h2>Message Types</h2>
 *
 * <ul>
 *   <li>{@link #showSorted(SortMethod, SortOrder)} - Displayed after a sort operation
 *   <li>{@link #showPreferenceChange(SortMethod, SortOrder)} - Displayed when preferences change
 * </ul>
 *
 * <h2>Rendering</h2>
 *
 ...
 *
 * <p>Feedback is purely visual and only exists on the client.
 *
 * @see ClientEvents
 * @since 1.0.0
 */
public class SortFeedback {
   ...
   *
   * <p>The message uses the translation key "message.bnnch_sort.sorted" with the method and order
   * as parameters.
   *
   ...
   *
   * <p>The message uses the translation key "message.bnnch_sort.preference_changed" with the new
   * method and order as parameters.
   *
  ...
  public static void showPreferenceChange(SortMethod method, SortOrder order) {
    Component message =
        Component.translatable(
            "message.bnnch_sort.preference_changed",
            Component.translatable(method.getTranslationKey()),
            Component.translatable(order.getTranslationKey()));
    displayOverlay(message);
  }
