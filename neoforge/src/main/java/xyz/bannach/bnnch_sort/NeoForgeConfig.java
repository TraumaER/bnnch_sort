...
/**
 * NeoForge-specific configuration management for the Bnnch: Sort mod.
 *
 * <p>This class owns the {@link ModConfigSpec} builders and event handlers. On load/reload it
 * writes the parsed values into {@link CommonConfig} so that platform-agnostic code can read them
 * without depending on NeoForge APIs.
 *
 * <h2>Client Configuration</h2>
 *
 * <ul>
 *   <li>{@link #SHOW_SORT_BUTTON} - Whether to display the sort button on container screens
 *   <li>{@link #LOCK_MODIFIER_KEY} - Modifier key used to toggle slot locks
 *   <li>{@link #LOCK_TINT_COLOR} - Tint color applied to locked slots (hex RGBA)
 *   <li>{@link #SHOW_LOCK_TOOLTIP} - Whether to show a tooltip on empty locked slots
 * </ul>
 *
 * <h2>Server Configuration</h2>
 *
 * <ul>
 *   <li>{@link #DEFAULT_SORT_METHOD} - Default sort method for new players
 *   <li>{@link #DEFAULT_SORT_ORDER} - Default sort order for new players
 * </ul>
 *
 * <h2>Side: NeoForge</h2>
 *
 ...
 * @since 1.0.0
 */
@EventBusSubscriber(modid = BnnchSort.MODID, bus = EventBusSubscriber.Bus.MOD)
public class NeoForgeConfig {
   ...
   *
   * <p>Updates {@link CommonConfig} fields when config files are loaded. Client and server configs
   * are handled separately based on the spec.
   *
   ...
   *
   * <p>Updates {@link CommonConfig} fields when config files are changed at runtime.
   *
  ...
  /**
   * Applies configuration values from the given spec to {@link CommonConfig}.
   *
   * @param spec the config spec that was loaded or reloaded
   */
  private static void applyConfig(Object spec) {
    if (spec == CLIENT_SPEC) {
      CommonConfig.showSortButton = SHOW_SORT_BUTTON.get();
      CommonConfig.lockModifierKey = LOCK_MODIFIER_KEY.get();
      CommonConfig.lockTintColor = parseColor(LOCK_TINT_COLOR.get(), 0x80FFD700);
      CommonConfig.showLockTooltip = SHOW_LOCK_TOOLTIP.get();
    } else if (spec == SERVER_SPEC) {
      CommonConfig.defaultSortMethod = DEFAULT_SORT_METHOD.get();
      CommonConfig.defaultSortOrder = DEFAULT_SORT_ORDER.get();
    }
  }
