 ...
 *
 * <p>This class contains NeoForge GameTest framework tests that verify the {@link ItemSorter} and
 * {@link SortHandler} classes work correctly. Tests cover all sort methods, stack merging, empty
 * slot handling, and menu slot partitioning.
 *
 * <h2>Test Categories</h2>
 *
 * <ul>
 *   <li>Sort method tests - Verify each comparator sorts correctly
 *   <li>Stack merging tests - Verify partial stacks are combined properly
 *   <li>Sort handler tests - Verify menu slot detection and sorting
 * </ul>
 *
 * <h2>Running Tests</h2>
 *
...
public class SortingGameTests {
  ...
  public static void merge_stacks_combines_partial_stacks(GameTestHelper helper) {
    ...
    helper.assertTrue(
        merged.getFirst().getCount() == 64,
        "Expected count 64, got " + merged.getFirst().getCount());
  ...
  public static void merge_stacks_overflows_to_new_stack(GameTestHelper helper) {
    ...
    helper.assertTrue(
        merged.get(1).getCount() == 26,
        "Second stack should be 26, got " + merged.get(1).getCount());
  ...
  public static void merge_stacks_does_not_merge_different_components(GameTestHelper helper) {
    ...
    helper.assertTrue(
        merged.size() == 2, "Expected 2 stacks (different components), got " + merged.size());
  ...
  public static void sort_handler_partitions_chest_slots(GameTestHelper helper) {
    ...
    helper.assertTrue(
        containerSlots.size() == 27, "Expected 27 chest slots, got " + containerSlots.size());
    helper.assertTrue(
        containerSlots.get(0).getItem().is(Items.STONE), "First slot should be stone");
    helper.assertTrue(
        containerSlots.get(1).getItem().is(Items.DIAMOND), "Second slot should be diamond");
    helper.assertTrue(
        containerSlots.get(2).getItem().is(Items.APPLE), "Third slot should be apple");
  ...
  public static void sort_handler_partitions_player_main_inventory(GameTestHelper helper) {
    ...
    helper.assertTrue(
        mainSlots.size() == 27, "Expected 27 main inventory slots, got " + mainSlots.size());
  ...
  public static void sort_handler_partitions_player_hotbar(GameTestHelper helper) {
    ...
    helper.assertTrue(
        hotbarSlots.size() == 9, "Expected 9 hotbar slots, got " + hotbarSlots.size());
   ...
   */
  @GameTest(template = "empty")
  public static void sort_handler_does_not_affect_player_inventory_when_sorting_chest(
      GameTestHelper helper) {
    Player player = helper.makeMockPlayer(GameType.SURVIVAL);
    BlockPos chestPos = new BlockPos(1, 1, 1);
    ...
    helper.assertTrue(
        player.getInventory().getItem(9).is(Items.DIAMOND), "Player slot 9 should be unchanged");
    helper.assertTrue(
        player.getInventory().getItem(10).is(Items.GOLD_INGOT),
        "Player slot 10 should be unchanged");
