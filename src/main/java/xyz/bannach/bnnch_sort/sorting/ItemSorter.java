 ...
 *
 * <p>This class provides the main sorting pipeline that handles stack merging, sorting, and slot
 * padding. It is designed to be stateless with all methods being static.
 *
 ...
 * <h2>Side: Common</h2>
 *
 * <p>This class is used primarily server-side for actual sorting, but the logic is available on
 * both sides.
 *
 * @see SortPreference
...
public final class ItemSorter {
   ...
   *
   * <p>The sorting process:
   *
   * <ol>
   *   <li>Merges partial stacks of the same item type
   *   <li>Removes empty stacks from consideration
   *   <li>Sorts remaining items using the preference's method
   *   <li>Reverses order if preference specifies descending
   *   <li>Pads with empty stacks to match original size
   * </ol>
   *
   ...
   *
   * <p>This method combines stacks that share the same item and components, respecting each item's
   * maximum stack size. Items that cannot be stacked (different items, different NBT, or max stack
   * size of 1) remain separate.
   *
   * <p>Example: Two stacks of 32 Stone become one stack of 64 Stone.
   *
