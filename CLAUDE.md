...
NeoForge mod for Minecraft 1.21.1 ("Bnnch: Sort"). Java 21, Gradle 8.8, NeoForge ModDev plugin v2.0.140. Parchment mappings for human-readable Minecraft names.
...
Game tests use the NeoForge GameTest framework (not JUnit). Tests are filtered by the mod namespace `bnnch_sort`. The `runGameTestServer` task will crash if no game tests are registered.

## Architecture

- **Mod entrypoint:** `src/main/java/xyz/bannach/bnnch_sort/BnnchSort.java` — annotated with `@Mod("bnnch_sort")`. Registers blocks, items, and creative tabs via NeoForge Deferred Registers.
- **Configuration:** `src/main/java/xyz/bannach/bnnch_sort/Config.java` — uses `ModConfigSpec` and `@EventBusSubscriber` for config loading.
...
- The mod ID `bnnch_sort` must stay aligned across `gradle.properties` (`mod_id`), the `@Mod` annotation, and `neoforge.mods.toml`.
